#!/usr/bin/env python3
"""R2 presign uploader for 0.9.6 (stdlib only, NO network in this script).
...
Usage: python _r2put096.py  ->  writes _r2urls.txt (2 lines: apk_url, json_url)
...
APK_PATH = os.path.join(REPO, "android", "app", "build2", "outputs", "apk", "debug", "PhotographerCamera-0.9.6.apk")
apk_size = os.path.getsize(APK_PATH)

version_payload = {
    "versionName": "0.9.6",
    "versionCode": 36,
    "url": "https://app.tybtool.top/PhotographerCamera-0.9.6.apk",
    "size": apk_size,
    "ts": int(time.time() * 1000),
    "notes": ("0.9.6: fix JPEG MAX/RAW MAX capture deadlock (mandatory burst sequence-finish "
              "anchor + 2s convergence - partial frames still save, zero-frame fast reset); "
              "zoom max is now telephoto lens x2 on every device (0.6/1/3x -> 0.6-6x, single "
              "camera -> 2x); frame-count sliders in settings (JPG max 3-20 default 5, HDR+ "
              "1-20 default 3); image reader margin +2; removed 297 dead-copy engine files; "
              "median main-camera baseline for lens zoom ratios"),
}
...
urls = [
    presign_put("PhotographerCamera-0.9.6.apk"),
