#!/usr/bin/env python3
"""R2 presign uploader for 0.9.4 (stdlib only, NO network in this script).

Derived from _r2put092.py. Builds version.json + presigned PUT URLs
(SigV4 query auth), then the shell does the actual transfer with curl -T.

Usage: python _r2put094.py  ->  writes _r2urls.txt (2 lines: apk_url, json_url)
...
APK_PATH = os.path.join(REPO, "android", "app", "build2", "outputs", "apk", "debug", "PhotographerCamera-0.9.4.apk")
apk_size = os.path.getsize(APK_PATH)

version_payload = {
    "versionName": "0.9.4",
    "versionCode": 34,
    "url": "https://app.tybtool.top/PhotographerCamera-0.9.4.apk",
    "size": apk_size,
    "ts": int(time.time() * 1000),
    "notes": ("0.9.4: fix 6 real-device bugs (capture save after camera disabled with "
              "auto recovery, live photo thumbnail delay, zoom limit now 2x native with "
              "digital crop fallback, viewfinder matches actual capture crop, live/raw "
              "toggle mutex, WB color temperature direction on ColorOS)"),
}
...
urls = [
    presign_put("PhotographerCamera-0.9.4.apk"),
