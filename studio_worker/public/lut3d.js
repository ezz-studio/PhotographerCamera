...
(function (global) {
  ...
  function bakeColorLUT(profile, n) {
    ...
    applyShadow(grid, sh.black_point || 0, sh.compression || 0, sh.saturation == null ? 1 : sh.saturation, sh.contrast == null ? 1 : sh.contrast);
  ...
  global.LUT3D = {
    bakeColorLUT, lut3dToCube, bakeToneLUT, bakeHslLUT,
    // 链函数导出：lut_pipe.js 组合 LUT→film→params 管线时复用同一份实现，
    // 保证 GL 预览 / .cube 导出 / JSON 烘焙三处数学完全一致。
    chain: {
      applyExposure, applyWB, applyColorMatrix, applyHighlight,
      applyShadow, applyFilmCurve, applyToneCurve, applyHSL,
    },
  };
})(window);
