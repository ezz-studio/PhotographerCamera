 ...
 * SHA-256: 0791fa8fdf096adab2e55a65c1b68b0f4b1e31529202eb0153f89882ee21b144
 * ELF offsets: 0x305adee
 * Symbols: hotPixelPlus_fragmentShaderSource
 */
...
uniform sampler2D uInputTexture0; // Image
uniform sampler2D uInputTexture1; // Mask
...
uvec4 readMask(ivec2 pos) {
    ivec2 mp = pos + ivec2(uParams.mapOfsX / 4, uParams.mapOfsY);
    return uvec4(round(texelFetch(uInputTexture1, mp, 0) * 255.0));
}

uvec4 doHotPixCluster(ivec2 pos, bool isRedRow) {
    vec4 sdRG; float rawMeanB, blueSd;
    vec4 rawMeanRG = GetRawMeanSD(uInputTexture0, pos, isRedRow, sdRG, rawMeanB, blueSd);
    vec4 meanRG = max(rawMeanRG - float(uParams.blacklevelOffset), uParams.colorMeanModerator);
    ...
    vec4 diffHi = max(max(sFactHi * sdRG, uParams.trivialDiffThr), meanRG * uParams.meanHighFact);
    vec4 thrMeanRGHi = rawMeanRG + diffHi;
    float2x4 ep = ExtraPolTest(uInputTexture0, pos, isRedRow, grRed, grBlue, true);

    uvec4 mL = uvec4(0), mT = uvec4(0), mR = uvec4(0), mB = uvec4(0);
    if (uParams.mapOfsX != 0 || uParams.mapOfsY != 0) {
        mT = readMask(pos + ivec2(0, -1)); mB = readMask(pos + ivec2(0, 1));
        uvec4 c = readMask(pos); uvec4 l = readMask(pos + ivec2(-1, 0)); uvec4 r = readMask(pos + ivec2(1, 0));
        mL = uvec4(l.w, c.x, c.y, c.z); mR = uvec4(c.y, c.z, c.w, r.x);
    }

    bvec4 nL = notEqual(mL, uvec4(eHpHot));
    bvec4 nT = notEqual(mT, uvec4(eHpHot));
    bvec4 nR = notEqual(mR, uvec4(eHpHot));
    bvec4 nB = notEqual(mB, uvec4(eHpHot));

    bvec4 mTest = bvec4(vec4(nL) * vec4(nT) * vec4(nR) * vec4(nB));

    ep.m1 *= vec4(mTest);

    vec4 thrHi = max(min(isRedRow?thrMeanRGHi:thrMeanRGHi.yxwz, 65535.0), ep.m1);
    vec4 diffLo = max(uParams.sDFactLo * sdRG, uParams.trivialDiffThr);
    vec4 thrMeanRGLo = min(vec4(float(uParams.blacklevelOffset) + meanRG * uParams.meanLowFact), rawMeanRG - diffLo);
    vec4 thrLo = min(isRedRow?thrMeanRGLo:thrMeanRGLo.yxwz, ep.m0);

    uvec4 dum;
    return MedianThrTest(uInputTexture0, pos, isRedRow, thrLo, thrHi, dum);
}
...
void main() {
    ...
    uvec4 outColor = doHotPixCluster(srcCoord, !leftmostisGreen);
