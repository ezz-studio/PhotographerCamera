...
class LivePhotoRecorder(
    ...
    /**
     * Fork 兼容 shim（0.9.9 整体移植上游后保留）：放弃一次尚未收敛的拍摄。
     * 上游已由 PendingCaptureStart 超时（CAPTURE_START_TIMEOUT_MS）与 recordVideo
     * 的错误回调覆盖大部分失败路径；本方法供 Camera2Controller 异常路径
     * （相机断开 / capture setup failure / single capture failed）调用，
     * 复位占用状态并释放保留区，防止录制器被永久占用。
     */
    fun cancelPendingCapture(reason: String) {
        pendingCaptureStart.getAndSet(null)?.let { pending ->
            isCapturing = false
            pending.ready.cancel()
        }
        // 保留区挂在 startCaptureAfterTorch 记录的起点上（captureMinimumTimestampUs），
        // 与 finishTorchRecording(cancelled=true) 的清理路径一致
        captureMinimumTimestampUs?.let(::releaseCaptureRetention)
        captureMinimumTimestampUs = null
        acceptingTorchFrames = false
        if (pendingRelease) {
        ...
        PLog.w(TAG, "Live Photo capture aborted: $reason")
    }
}
