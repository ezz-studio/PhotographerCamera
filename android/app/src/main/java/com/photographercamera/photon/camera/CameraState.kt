...
import com.photographercamera.photon.video.VideoCapabilities
import com.photographercamera.photon.video.VideoConfig
...
data class CameraInfo(
    ...
    fun getBoundPhysicalCameraId(zoomRatioByMain: Float): String? {
        ...
        // ≥2.95 绑长焦、<0.95 绑超广角（1.3.0 从 1.2.3 的 0.99 回退——1.2.3 提前
        // 绑定是为缓解逻辑机默认形态的切镜观感，1.3.0 默认镜头已回退独立主摄，
        // 本绑定机制仅手动打开逻辑机时兜底，恢复 1.2.2 之前的进入阈值）、1x 附近
        // 走逻辑流；退出滞回在 resolveRequestedPhysicalCameraId
        // （基准 activeOutputPhysicalCameraId）。
