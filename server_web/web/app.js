/* PhotographerCamera Studio — local-only JSON tuning console.
 *
 * No server round-trips for compute or images:
 *   - photos are read from the user's machine (folder/file picker or drag-drop)
 *     and kept in browser memory as decoded canvases
 *   - the preview is rendered on the client GPU via GLRender (gl_lut.js),
 *     which mirrors the desktop reference pipeline (profile_renderer.py)
 *   - profile import / save / LUT export all happen in-browser
 *
 * The training / "generate new style from reference images" flow and the
 * validation-set UI have been removed — this page only tunes an existing JSON.
 */
...
const GROUP_LABEL = {
  ...
  film_curve: "胶片曲线", hsl: "HSL 分色", sharpen: "锐化", bloom: "柔光溢出 Bloom",
  halation: "光晕 Halation", grain: "颗粒 Grain", noise: "噪点 Noise",
  vignette: "暗角 Vignette", lens: "镜头 Lens",
};
const PARAM_LABEL = {
...
function defaultProfile(name) {
  return {
    version: 1, schema_version: 1, name: name || "My Look",
...
function loadProfile(prof, name) {
  ...
  if (merged.display) {
    $("#profileName").value = merged.display.name || name || "My Look";
    $("#profileIntro").value = merged.display.intro || "";
  } else {
    $("#profileName").value = merged.name || name || "My Look";
  }
  renderControls();
...
function saveProfile() {
  ...
  const prof = clone(state.profile);
...
function init() {
  ...
  $("#btnNew").onclick = () => {
    loadProfile(defaultProfile($("#profileName").value.trim() || "My Look"));
