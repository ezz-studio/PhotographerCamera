...
void main(){
  ...
  vec3 c = src;
  if (u_useLut > 0.5) {
    // Learned 3D LUT + film curve — the colour chain ENDS here, exactly like
    // the authoritative profile_renderer.render() LUT branch: parametric
    // colour settings (exposure/wb/matrix/highlight/shadow/tone/HSL) are the
    // stylefit *approximation* of the same look and are BYPASSED for LUT
    // profiles. Applying both would style the image twice (blown-out look).
    // Coordinate remap: CPU lattice at c*(n-1) <=> GL texel centres at (i+.5)/n.
    vec3 coord = (clamp(src, 0.0, 1.0) * 32.0 + 0.5) / 33.0;
